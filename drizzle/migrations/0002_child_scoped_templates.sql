...
-- Backfill child_id based on naming convention (-A vs -B)
...
WHERE
    name LIKE '%-B';
-- For any remaining without suffix default to first child (adjust if needed)
UPDATE schedule_template
SET
    child_id = (
        SELECT id
...
-- Create template_subject_material
...
-- Migrate existing global subject_material into template scoped for each template that contains the subject
-- (Only if subject_material table exists)
